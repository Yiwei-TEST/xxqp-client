 ...
 */
var ERDDZReplayCtrLayer = cc.Layer.extend({
    step:0,
    isPause:false,
    _dt:0,
    ctor:function(){
    ...
    initLayer:function(){
        ...
        this.bg_btn.setPosition(cc.winSize.width/2,cc.winSize.height/2 + 75);
        ...
        this.label_step.setPosition(cc.winSize.width/2,cc.winSize.height/2 + 180);
    ...
    setShowStep:function(){
        this.label_step.setString("进度:" + (this.step) + "/" + ERDDZReplayMgr.msgArr.length);
    },

    autoPlay:function(){
        if(this.step < ERDDZReplayMgr.msgArr.length - 1){
            this.step++;
            ERDDZReplayMgr.sendPlayMsg(this.step);
            this.setShowStep();
        }else if(this.step == ERDDZReplayMgr.msgArr.length - 1){
            ERDDZReplayMgr.sendOverMsg();
    ...
    onClickBtn:function(sender,type){
        if(type == ccui.Widget.TOUCH_BEGAN){
            ...
            if(sender == this.btn_back){
                ...
                if(this.step < ERDDZReplayMgr.tablesArr.length - 1){
                    this._dt = 0;

                    this.step += 4;
                    if(this.step >= ERDDZReplayMgr.tablesArr.length){
                        this.step = ERDDZReplayMgr.tablesArr.length - 1;
                    }

                    ERDDZReplayMgr.sendTableMsg(this.step);
                    this.setShowStep();
