...
var XPLPReplayLayout = cc.Class.extend({

    initData:function(direct,mPanel,oPanel,hPanel,seat){
        if (XPLPReplayModel.renshu == 2 && direct == 2){
            direct = 3;
        }
        if (XPLPReplayModel.renshu == 3 && direct == 3){
            direct = 4;
        }
        this.direct = direct;
        this.seat = seat || null;
    ...
    moPai:function(id){
        ...
        mjVo.m = 1;
        if(XPLPReplayModel.intParams[4] == 1 && XPLPReplayModel.guchouSeat != -1){//当前有玩家箍臭
    ...
    showFinger:function(id){
        ...
        finger.x = lastMahjong.x+lastMahjong.height/2 - 30;
        var initY = lastMahjong.y+lastMahjong.width/2 - 30;
        finger.y = initY;
        finger.visible = true;
    ...
    pengPai:function(ids,action,fromSeat,isZimo){
        ...
        var vo0 = voArray[0];
        var isZimo = isZimo || -1;
        if((action==XPLPAction.GANG||action==XPLPAction.BU_ZHANG) && ids.length==1){//先碰后杠
        ...
        if(action == XPLPAction.HU){
            this.refreshBaiJiaoCards(this.data2);
        }
        if(action!=XPLPAction.AN_GANG && action!=XPLPAction.BU_ZHANG && action != XPLPAction.CHI
            && ids.length>1)
            ids.pop();
    ...
    showJinCards:function(ids){
        var jinArray = [];
        for(var i=0;i<ids.length;i++){
            jinArray.push(XPLPAI.getMJDefByI(ids[i]));
        }
        for(var i=0;i<this.data1.length;i++){
            var cardVo = this.data1[i];
            var index = XPLPAI.findIndexByMJVoI(jinArray, cardVo.i);
            if (index >= 0) {
                cardVo.jinDisplay = 1;
            }
        }
    ...
    showBaiJiaoCards:function(ids){
        var cards = XPLPAI.getVoArray(ids);
    ...
    refreshBaiJiaoCards:function(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            for (var j = 0; j < innerArray.length; j++) {
                ...
                if(paoSeat > 0 && innerAction == XPLPAction.HU){
                    var paoSeq = XPLPReplayModel.getPlayerSeq("", XPLPReplayModel.mySeat, paoSeat);
                    innerVo.jt = XPLPReplayModel.getPaoJianTou(paoSeq);
                }
                if (innerAction == XPLPAction.BAI_JIAO || innerAction == XPLPAction.HU ) {
                    var mod = 1;
                    if (count < nowCards) {
                    ...
                    if (innerAction == XPLPAction.BAI_JIAO && !card.getChildByTag(10000)) {
    ...
    onAddBaiZi: function (card) {
        var png = "res/res_mj/res_bsmj/bsmjRoom/bai_down.png";
        if(this.direct == 2 || this.direct == 4){
            png = "res/res_mj/res_bsmj/bsmjRoom/bai_left.png";
        }
        ...
        var baiZi = new cc.Sprite("res/res_mj/res_bsmj/bsmjRoom/bai.png");
    ...
    refreshP1:function(data){
        ...
        var tempSize = 95;
        var localNum = 17;
        var gapMapping = {1:tempSize,2:tempSize,3:tempSize,4:tempSize};
        var g=gapMapping[this.direct];
        ...
        for(var i=0;i<this.mahjongs1.length;i++){
            ...
            zorder--;
            card.refresh(XPLPAI.getDisplayVo(this.direct,place),newData[i]);
        }
        for(;i<newData.length;i++){
            var card = new XPLPCard(XPLPAI.getDisplayVo(this.direct,place),newData[i]);
    ...
    refreshP2:function(data){
        ...
        switch (this.direct){
            ...
            case 4:
                g = 95;
                initVal = 580 - (XPLPAI.MJ_NUMBER-this.data2.length-1)*7;
                break;
        }
        ...
        for(var i=0;i<data.length;i++) {
            ...
            var innerArray = innerObject.cards;
            var gangVo = null;
            if((innerAction==XPLPAction.AN_GANG||innerAction==XPLPAction.GANG) && (innerArray.length>3 || innerObject.temp)){
                if(innerObject.temp){
            ...
            for(var j=0;j<innerArray.length;j++) {
                ...
                if (innerAction != XPLPAction.BAI_JIAO && innerAction != XPLPAction.HU) {
                    ...
                    if (this.direct == 2) {
                        card.y = initVal + count * g * localScale + 15 * mod;
                        card.setLocalZOrder(zorder);
                    } else if (this.direct == 4) {
                        card.y = initVal - count * g * localScale - 15 * mod;
                    } else {
        ...
        if(this.direct==2)
            this.p2Mark = isHasData2 ? lastY+60 : initVal+this.data2.length*5;
        if(this.direct==3)
            this.p2Mark = isHasData2 ? lastX-60 : initVal;
        if(this.direct==4)
    ...
    refreshP3:function(data){
        ...
        if (XPLPReplayModel.renshu == 2){
            rowCount = 20;
        }else if(XPLPReplayModel.renshu == 3 && this.direct==1){
            rowCount = 18;
        }else if(XPLPReplayModel.renshu == 3 && this.direct!=1){
            rowCount = 5;
        }else if(XPLPReplayModel.renshu == 4 && (this.direct == 2 || this.direct == 4)){
            rowCount = 5;
        }
        for(;i<data.length;i++){
            var card = new XPLPCard(XPLPAI.getDisplayVo(this.direct,3),data[i]);
    ...
    refreshP4: function(data) {
        ...
        for(var i=0;i<data.length;i++) {
            ...
            var huRecord = XPLPReplayModel.getHuRecordById(this.seat,vo.c);
            if (huRecord && huRecord.ext[1] > 0) {
                var paoSeq = XPLPReplayModel.getPlayerSeq("", XPLPReplayModel.mySeat, huRecord.ext[1]);
                vo.jt = XPLPReplayModel.getPaoJianTou(paoSeq);
            }
            var scale = 0.9;
