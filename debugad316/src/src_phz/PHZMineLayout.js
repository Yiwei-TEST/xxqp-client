...
setRoot:function(root){
    ...
    getCardOffX:function(){
        var w = PHZSetModel.zpdx == 1 ? 114 : 126; //75
    ...
    swapCardsPos: function (fromIndexArr, toIndexArr) {
        // cc.log("fromIndexArr =", JSON.stringify(fromIndexArr));
        // cc.log("toIndexArr =", JSON.stringify(toIndexArr));
        var tempCard = this.curSortedData[fromIndexArr.x][fromIndexArr.y];
        var targetCard = this.curSortedData[toIndexArr.x][toIndexArr.y];
        this.curSortedData[fromIndexArr.x][fromIndexArr.y] = targetCard;
        this.curSortedData[toIndexArr.x][toIndexArr.y] = tempCard;
    },

    fixCardAni:function(fixPosNode){
        fixPosNode.runAction(cc.sequence(cc.moveTo(0.1 , fixPosNode.realX , fixPosNode.realY),cc.callFunc(function(target){
            target.initCoord();
            target.setCardIndex(target.realIndexX, target.realIndexY);
            target.setLocalZOrder(target.realZorder);
            if(target.isLastOne){//最后一个卡牌移动完动作 则放开操作锁
                //if(PHZMineLayout.isLocked()){
                //    PHZMineLayout.unlockCard();
