...
var CQXZMJLayout = cc.Class.extend({
    gangPai:[],
    ...
    getHuanzhangCards:function(){
        var idArray = [];
        if(this.mahjongs1.length >= 13){
            for(var j=0;j<this.mahjongs1.length;j++){
                var mahjongs = this.mahjongs1[j];
                var curVo = mahjongs.getData();
                if(mahjongs.isMoveUp()){
                    idArray.push(curVo.c);
                }
            }
        }
        return idArray;
    },
    huanzhang_cards_up:function(array) {
        for (var i = 1; i < array.length; i++) {
            for(var j=0;j<this.mahjongs1.length;j++){
                ...
                if(curVo.c == array[i]){
                    mahjongs.moveUp();
                    MJRoomModel.cqxzmj_huanzhangCardsType = curVo.t;
                    break;
                }
            }
        }
        this.check_cardsGray();
    },
    showHuanZhangAni:function(arr){
        var array = arr.split(",");
        for (var i = 0; i < array.length; i++) {
            for(var j=0;j<this.mahjongs1.length;j++){
                var mahjongs = this.mahjongs1[j];
                var curVo = mahjongs.getData();
                mahjongs.removeGreyBg();
                if(curVo.c == array[i]){
                    // cc.log("curVo.c =",curVo.c);
                    mahjongs.moveUp();
                    var action =cc.sequence(cc.delayTime(0.1),cc.moveTo(0.5,mahjongs.initX,mahjongs.initY));
                    mahjongs.runAction(action);
                }
            }
        }
    },
    huanzhang_sucess:function(array){
        cc.log("array =",JSON.stringify(array));
        MJRoomModel.cqxzmj_huanzhangCardsType = -1;
        var self = this;
        var num = 0;
        for (var i = 0; i < array.length; i++) {
            for(var j=0;j<this.mahjongs1.length;j++){
                ...
                if(curVo.c == array[i]){
                    num += 1;
                    var action = cc.spawn(cc.moveTo(0.4,cc.winSize.width/2,400),cc.scaleTo(0.4,0,0));
                    if(num == 3){
                        action = cc.sequence(cc.spawn(cc.moveTo(0.4,cc.winSize.width/2,400),cc.scaleTo(0.4,0,0)),
                            cc.callFunc(function(){
                                for(var k = 0; k < array.length; k++){
                                    self.delFromPlace1(array[k]);
                                }
                            }));
                    }
    ...
    xiaohu:function(ids){
        ...
        for(var i=0;i<data.length;i++){
            var card = new CQXZMahjong(MJAI.getDisplayVo(this.direct,3),data[i]);
            if(this.direct==1){
                card.x = initVal+i*g;
     ...
     */
    refreshP1:function(data,isShowHuanzhangAni,huanZhangCards){
        ...
        for(var j=0;j<data.length;j++){
            if (data[j].t != MJRoomModel.cqxzmj_dingqueType && data[j].m != 1 ){
                newData.push(data[j]);
            }
        }
        for(var j=0;j<data.length;j++){
            if ( data[j].m == 1 || data[j].t == MJRoomModel.cqxzmj_dingqueType){
                newData.push(data[j]);
            }
        }
        var gapMapping = {1:133,2:41,3:66,4:41};//{1:88,2:34,3:44,4:34};
        ...
        for(;i<newData.length;i++){
            newData[i].feiDisplay = 1;
            var card = new CQXZMahjong(MJAI.getDisplayVo(this.direct,1),newData[i]);
            // card.removeGreyBg();
        ...
        this.check_cardsGray();
        if(isShowHuanzhangAni){
            this.showHuanZhangAni(huanZhangCards)
        }
    },

    check_cardsGray:function(){
        var num = [0,0,0,0];
        var check_num = MJRoomModel.intParams[5]?4:3;
        
        if(MJRoomModel.cqxzmj_gameType == 1){//换张
            for(var i=0;i<this.mahjongs1.length;i++){
                var card = this.mahjongs1[i];
                num[card._cardVo.t] += 1;
            }
            for (var i = 1; i < num.length; i++) {
                if(num[i] < check_num){
                    for(var j=0;j<this.mahjongs1.length;j++){
                        var card = this.mahjongs1[j];
                        if(card._cardVo.t == i){
                            card.displayGreyBg(true);
                        }
                    }
                }
            }
        }else if(MJRoomModel.cqxzmj_gameType == 3){//定缺后 定缺的牌先打
            var num_dingque = 0;
            for(var i=0;i<this.mahjongs1.length;i++){
                var card = this.mahjongs1[i];
                if(card._cardVo.t == MJRoomModel.cqxzmj_dingqueType){
                    num_dingque += 1;
                }
            }
            ...
            for(var j=0;j<this.mahjongs1.length;j++){
                var card = this.mahjongs1[j];
                if(num_dingque>0 && card._cardVo.t != MJRoomModel.cqxzmj_dingqueType){
                    card.displayGreyBg(true);
                }else if (num_dingque == 0){
                    card.removeGreyBg();
                }
            }
        ...
        for(var i=0;i<data.length;i++) {
            ...
            for(var j=0;j<innerArray.length;j++){
                ...
                if(count<nowCards){
                    ...
                    card = new CQXZMahjong(MJAI.getDisplayVo(this.direct,2),innerVo);
                    this.mPanel.addChild(card);
                }
                ...
                if(gangVo && j==1){
                    if(!card.getChildByTag(333)){
                        ...
                        //    gangVo.a = 1;
                        var gang = new CQXZMahjong(MJAI.getDisplayVo(this.direct,2),gangVo);
                        if(this.direct==1)
                            gang.y = 20;
                        if(this.direct==2)
                            gang.y += 15;
    ...
    refreshP4: function(data) {
        ...
        for(var i=0;i<data.length;i++) {
            ...
            vo.huDisplay = 1;
            var card = new CQXZMahjong(MJAI.getDisplayVo(this.direct,4),vo);
            card.x = initVal+g*i;
            card.y = 0;
            if(this.direct==1){
                card.x = initVal-i*g;
