 ...
 */
var ZJMJLayout = cc.Class.extend({
    gangPai:[],
    ...
    getCardNumById:function(vo){
        for(var j = 0;j < this.data2.length;++j){
            ...
            if(tempData.action == MJAction.GANG || tempData.action == MJAction.AN_GANG ||
                tempData.action == MJAction.YAO_GANG || tempData.action == MJAction.YAO_ANGANG){
    ...
    pengPai:function(ids,action,fromSeat){
        ...
        }else if((action==MJAction.GANG||action==MJAction.YAO_GANG) && ids.length==1){//先碰后杠
            for(var i=0;i<this.data2.length;i++){
                var innerObj = this.data2[i];
        ...
        if(action==MJAction.CHI){
            ...
            if(action!=MJAction.AN_GANG && action!=MJAction.BU_ZHANG && action!=MJAction.YAO_ANGANG
                && ids.length>1)
                ids.pop();
    ...
    calcData2Length:function(){
        ...
        for(var i=0;i<this.data2.length;i++){
            ...
            switch (action){
                ...
                case MJAction.YAO_GANG:
                case MJAction.YAO_ANGANG:
                    length+=3;
                    break;
                default :
                    length+=cards.length;
    ...
    refreshP1:function(data){
        ...
        //    if(MJRoomModel.isYAOGANG){/** 摇杠锁牌 **/
        //        newData[i].isYaoGang = true;
        //        if(newData.length % 3 == 2){//是自己出牌
        //            if(i === newData.length - 1){
        //                newData[i].isYaoGang = false;/** 摸的那张牌不锁 */
        //            }
        //        }
        //    }
        //}
    ...
    refreshP2:function(data){
        ...
        for(var i=0;i<data.length;i++) {
            ...
            var gangVo = null;
            if((innerAction==MJAction.AN_GANG||innerAction==MJAction.GANG ||innerAction==MJAction.BU_ZHANG
            ...
            if(innerAction==MJAction.YAO_GANG || innerAction==MJAction.YAO_ANGANG){/** 如果有摇杠 */
                hasYaoGang = true;
            }
        ...
        if(this.direct==1 && !MJRoomModel.isYAOGANG && hasYaoGang){
            MJRoomModel.isYAOGANG = true;/** 摇杠 */
        }
    },

    isContainIds:function(ids){
         ids = ids || [];
         var isHas = true;
         var handIds = this.getPlace1Ids();
         for(var i = 0;i < ids.length;++i){
             if(handIds.indexOf(ids[i]) === -1){
                 isHas = false;
                 break;
             }
         }
    ...
    getPlace1Ids:function(){
        var ids = [];
        for(var i = 0;i < this.data1.length;++i){
            ids.push(this.data1[i].c);
        }
        return ids;
    },
